...
--local MOUSE_SENSITIVITY = Vector2.new( 0.002 * math.pi, 0.0015 * math.pi ) -- original
local MOUSE_SENSITIVITY = Vector2.new( 0.002 * math.pi, 0.0015 * math.pi)/4.5
local MOUSE_SENSITIVITY_NORMAL = MOUSE_SENSITIVITY
local MOUSE_SENSITIVITY_ADS = Vector2.new( 0.002 * math.pi, 0.0015 * math.pi)/9
...
local needsVehicleMouse = game.Players.LocalPlayer:WaitForChild("NeedsVehicleMouse")
local mouseLockOffsetObj = script.Parent:WaitForChild("MouseLockController"):WaitForChild("CameraOffset")
...
function BaseCamera:ConnectInputEvents()
	...
	if UserInputService.KeyboardEnabled then
		local function fixRightClickBug()
			if button2InputObject then
				self:OnMouse2Down(button2InputObject, false)
				self:OnMouse2Up(button2InputObject, false)
			end
		end
		self.hasToolOutConn = hasToolOut.Changed:Connect(fixRightClickBug)
		self.needsVehicleMouseConn = needsVehicleMouse.Changed:Connect(fixRightClickBug)
	end
	self.mouseLockOffsetChangedConn = mouseLockOffsetObj.Changed:Connect(function(newOffset)
		if newOffset == ZERO_VECTOR3 then
			MOUSE_SENSITIVITY = MOUSE_SENSITIVITY_NORMAL
		else
			MOUSE_SENSITIVITY = MOUSE_SENSITIVITY_ADS
		end
		self:SetMouseLockOffset(newOffset)
	end)
...
function BaseCamera:DisconnectInputEvents()
	--Kingedpawn
	if self.hasToolOutConn then
		self.hasToolOutConn:Disconnect()
	end
	if self.needsVehicleMouseConn then
		self.needsVehicleMouseConn:Disconnect()
	end
	if self.mouseLockOffsetChangedConn then
		self.mouseLockOffsetChangedConn:Disconnect()
	end
...
function BaseCamera:OnMouseMoved(input, processed)
	...
	if hasToolOut.Value or (self.panEnabled and ((self.startPos and self.lastPos and self.panBeginLook) or self.inFirstPerson or self.inMouseLockedMode)) then
		local desiredXYVector = self:InputTranslationToCameraAngleChange(inputDelta,MOUSE_SENSITIVITY)
		self.rotateInput = self.rotateInput + desiredXYVector
	end
	--]]
	if not needsVehicleMouse.Value or (self.panEnabled and ((self.startPos and self.lastPos and self.panBeginLook) or self.inFirstPerson or self.inMouseLockedMode)) then
		local desiredXYVector = self:InputTranslationToCameraAngleChange(inputDelta,MOUSE_SENSITIVITY)
		self.rotateInput = self.rotateInput + desiredXYVector
		self.lastUserPanCamera = tick()
	end
...
function BaseCamera:UpdateMouseBehavior()
	...
	if self.inFirstPerson or self.inMouseLockedMode then
		...
		if self.isRightMouseDown or self.isMiddleMouseDown then
			...
			--KingedPawn
			UserInputService.MouseBehavior = not needsVehicleMouse.Value and Enum.MouseBehavior.LockCenter or Enum.MouseBehavior.Default
			--UserInputService.MouseBehavior = Enum.MouseBehavior.LockCenter 
			--hasToolOut.Value and Enum.MouseBehavior.LockCenter or Enum.MouseBehavior.Default
		end
	end
end
