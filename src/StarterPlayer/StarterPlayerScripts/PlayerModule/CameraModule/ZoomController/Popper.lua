...
local IGNORE_CAMERA_TAG = "IgnoreCamera"
...
local blacklist = {} do
	...
	local function refreshIgnoreList()
		blacklist = {unpack(Collection:GetTagged(IGNORE_CAMERA_TAG))}
		local n = #blacklist + 1
	...
	Collection:GetInstanceAddedSignal(IGNORE_CAMERA_TAG):Connect(OnIgnoreCollectionUpdated)
	Collection:GetInstanceRemovedSignal(IGNORE_CAMERA_TAG):Connect(OnIgnoreCollectionUpdated)
	Players.PlayerAdded:Connect(playerAdded)
	Players.PlayerRemoving:Connect(playerRemoving)
	
	for _, instance in next,Collection:GetTagged(IGNORE_CAMERA_TAG) do
...
local function getCollisionPoint(origin, dir)
	...
	repeat
		...
		if hitPart then
			if hitPart.CanCollide and not Collection:HasTag(hitPart,IGNORE_CAMERA_TAG) then
